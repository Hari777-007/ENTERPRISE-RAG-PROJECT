...
def generate_answer(state: GraphState) -> dict:
    ...
    if intent == "sql":
        ...
        return {
            "final_answer": answer,
            "sources": ["database query"],
            "confidence": 0.9, # why 0.9 because the sql query will be more accurate while giving the results.
        }
    ...
    response = run_rag(state["question"], flags=state.get("flags", {}))
    chunk_previews = [ # it helps to add meta data for queries and it can be shown in streamlit
...
def _get_checkpointer():
    ...
    saver.setup() # this setups the table in postgres for saving the checkpointer of each node's state
    return saver
