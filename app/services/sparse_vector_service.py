...
class SparseVectorIndex:
    def __init__(self) -> None:
        ...
        self.matrix = None
        self._lock = threading.RLock()# allows only the same thread to acquire the lock multiple times, preventing deadlocks in recursive calls
    ...
    def fit(self, documents: list[dict]) -> None:
        with self._lock:
            ...
            try:
                self.matrix = self.vectorizer.fit_transform(texts) # prepares the TF-IDF matrix(sparse vectors) for the documents
            except ValueError:
                self.matrix = None
                return
...
def fuse_rrf(
    ...
    for result_list in result_lists:
        for rank, chunk in enumerate(result_list):
            key = chunk.text
            scores[key] = scores.get(key, 0.0) + 1.0 / (rrf_k + rank + 1) # reciprocal rank fusion formula
            if key not in meta:
