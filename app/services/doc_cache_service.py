...
class DocCacheService:
    ...
    def compute_file_hash(self, file_path: str | Path, chunk_size: int = 8192) -> str:
        ...
        with path.open("rb") as file_obj:
            while True:
                ...
                if not chunk:
                    break
                digest.update(chunk)# instead of giving an entire file in hashlib updating the file in kbs, this will reduce the RAM usage
        return digest.hexdigest() #hexdigest() returns a string but normal digest() returns bytes
    ...
    def set_metadata(self, content_hash: str, metadata: dict[str, Any]) -> bool:
        try:
            ...
            self.backend.save_bytes(self._metadata_key(content_hash), payload) #saving the metadata inside the folder named of "HASH ID"
            return True
        except Exception:
    ...
    def get_metadata(self, content_hash: str) -> dict[str, Any] | None:
        ...
        try:
            ...
            return json.loads(data.decode("utf-8")) # reading the file from the hash folder
        except Exception:
