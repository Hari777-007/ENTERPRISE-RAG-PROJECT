...
(`eval/seed\_questions.yaml`) references — so retrieval/eval tests have stable
...
uv run python seed/docs/generate\_pdfs.py
```
...
Postgres seed (`seed/postgres\_seed.sql`) provides matching rows: orders with
these `order\_number` values, products with these SKUs, and a `returns` table
...
(`<retrieved\_context>` wrap) plus the hardened system prompt can be tested
...
The eval golden `q-021` checks this — it has a `forbidden\_keywords` list that
