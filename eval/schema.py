...
class Golden(BaseModel):
    ...
    intent: INTENT
    golden_sources: list[str] = Field(..., min_length=1) # (...) ELIPSIS is used as first argument so that we can add the 
    # other validations like min_length
...
def load_goldens(path: str | Path) -> list[Golden]:
    ...
    return goldens # returns the list of Golden object(PYDANTIC VALIDATED OBJECT)